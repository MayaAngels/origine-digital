// lib/homepage/use-live-data.ts
// React hooks for fetching live ORIGINE.DIGITAL data
...
interface AuditData {
    ...
    audit: { consistencyScore: number; totalDecisions: number };
}
...
export function useConsciousnessField(refreshInterval = 8000) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(`${BASE_URL}/api/reality/consciousness`);
...
export function useProducts(refreshInterval = 15000) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(`${BASE_URL}/api/revenue/activate`);
                const json = await res.json();
...
export function useAuditData(refreshInterval = 30000) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(`${BASE_URL}/api/intelligence/audit`);
                const json = await res.json();
                setData(json);
            } catch (e) { /* silent */ }
        };
