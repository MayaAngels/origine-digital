                ...
                <Grid cols={2} gap="lg" className="items-center">
                    ...
                    <div className="space-y-6">
                        <Badge variant="ai" dot>
                            {sources} AI Data Sources Active
                        </Badge>
                        <H1>
                            The{' '}
                            ...
                            that runs your digital business while you sleep.
                        </H1>
                        <Body className="text-base!">
                            ...
                            a mathematically proven ethical core. <strong className="text-[#E6EAF0]">No team required.</strong>
                        </Body>
                    ...
                    <div className="bg-[#11151A] border border-[rgba(255,255,255,0.06)] rounded-2xl p-6 space-y-4 shadow-[0_8px_24px_rgba(0,0,0,0.25)]">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-[#6EE7B7] uppercase tracking-wider">Live Intelligence</span>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <StatBox label="Data Sources" value={sources.toString()} />
                            <StatBox label="Awareness" value={awareness.toFixed(2)} />
                            <StatBox label="Trust Score" value={trustScore.toFixed(2)} color="green" />
                            <StatBox label="Decisions Today" value="12,487" />
                        </div>
                        <p className="text-[0.65rem] text-[#5A6378] text-center pt-2 border-t border-[rgba(255,255,255,0.06)]">
                            Bayesian pricing · Causal-aware bandits · Ethical vector on every action
                        </p>
                    </div>
                </Grid>
            </Container>
...
function StatBox({ label, value, color = 'green' }: { label: string; value: string; color?: string }) {
    ...
    return (
        <div className="bg-[#0B0D10] rounded-xl p-3 text-center">
            <div className="text-xl sm:text-2xl font-extrabold tracking-tight" style={{ color: colorMap[color] || '#6EE7B7' }}>
                {value}
            </div>
            <div className="text-[0.6rem] text-[#5A6378] uppercase tracking-wider mt-0.5">{label}</div>
        </div>
    );
}
