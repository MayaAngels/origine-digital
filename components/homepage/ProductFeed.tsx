            ...
            <Container>
                ...
                <Grid cols={3} gap="md">
                    {products.map((product) => {
                        ...
                        return (
                            <a key={product.id} href={`/checkout?product=${product.id}&price=${product.price}&title=${encodeURIComponent(product.title)}`} className="bg-[#11151A] border border-[rgba(255,255,255,0.06)] rounded-2xl overflow-hidden hover:border-[rgba(255,255,255,0.12)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.25)] transition-all duration-180 group block">
                                ...
                                <div className="p-4">
                                    <div className="flex items-center justify-between mb-2">
                                        <Badge variant={badge.variant} dot>Reality Score {badge.label}</Badge>
                                    ...
                                    <div className="flex flex-wrap gap-1.5 mb-3">
                                        {product.tags.slice(0, 3).map((tag) => <span key={tag} className="text-[0.6rem] px-2 py-0.5 rounded-full bg-[rgba(167,139,250,0.08)] text-[#A78BFA] font-medium">{tag}</span>)}
                                    </div>
...
function getProductEmoji(tags: string[]): string {
    const emojiMap: Record<string, string> = { 'AI': '🤖', 'Business': '📘', 'Ireland': '🇮🇪', 'Automation': '⚡', 'Entrepreneur': '🧰', 'Templates': '📋', 'Social Media': '📱', 'Content': '✍️', 'Marketing': '📊', 'Freelance': '💼', 'Tax': '🧾', 'Finance': '💰', 'Student': '🎓', 'Income': '💡', 'Budget': '📊' };
