...
export default function Badge({ children, variant = 'default', className = '', dot = false }: { children: React.ReactNode; variant?: BadgeVariant; className?: string; dot?: boolean }) {
    return <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[0.65rem] font-semibold rounded-full ${variantStyles[variant]} ${className}`}>{dot && <span className="w-1.5 h-1.5 rounded-full bg-current animate-pulse" />}{children}</span>;
}
